...
public class CinemaForYouClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        // 1c. 注册"打开影院界面"按键（可在原版 选项→控制→按键绑定 中自定义改键）
        // 26.2：KeyMapping 需经 Fabric KeyBindingHelper 注册才会出现在
        // 原版按键绑定列表里（Options.keyMappings）。
        // 默认 V：单独按 = 对着屏幕直接打开该屏控制（无目标则打开全局设置）；
        // Shift+V = 无论距离/位置都先打开"选择屏幕"列表再指定屏幕。
        openSettingsKey = new KeyMapping("key.cinemaforyou.open",
                InputConstants.Type.KEYSYM,
                GLFW.GLFW_KEY_V,
                KeyMapping.Category.MISC);
        net.fabricmc.fabric.api.client.keymapping.v1.KeyMappingHelper.registerKeyMapping(openSettingsKey);
        ...
        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            ...
            // 自定义按键（默认 V）：无 GUI 时响应。
            // Shift+按键 → 打开"选择屏幕"列表（任意距离/位置）；
            // 单独按键 → 对着屏幕（16 格内）直接打开该屏控制，否则全局设置。
            if (openSettingsKey != null && openSettingsKey.consumeClick()
                    && client.gui.screen() == null && client.player != null) {
                // 26.2 无 Screen.hasShiftDown，用 GLFW 直接查询修饰键状态
                boolean shift = false;
                try {
                    long win = client.getWindow().handle();
                    shift = org.lwjgl.glfw.GLFW.glfwGetKey(win, org.lwjgl.glfw.GLFW.GLFW_KEY_LEFT_SHIFT)
                                    == org.lwjgl.glfw.GLFW.GLFW_PRESS
                            || org.lwjgl.glfw.GLFW.glfwGetKey(win, org.lwjgl.glfw.GLFW.GLFW_KEY_RIGHT_SHIFT)
                                    == org.lwjgl.glfw.GLFW.GLFW_PRESS;
                } catch (Throwable ignored) {}
                if (shift) {
                    client.gui.setScreen(new com.cinemaforyou.client.gui.ScreenPickerScreen());
                } else {
