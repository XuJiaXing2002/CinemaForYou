...
public abstract class ScrollableSettingsScreen extends Screen {
    ...
    /** 打开子界面并记录当前页为上级（关闭子页时回到本页）。 */
    protected void openChild(Screen child) {
    ...
    /**
     * 统一分页条（与本地视频库一致）：[◀] [page/total] [▶]。
     *
     * @return 绘制后的下一个 y
     */
    protected int addPager(int left, int y, int page, int totalPages,
                           java.util.function.BooleanSupplier canPrev,
                           java.util.function.BooleanSupplier canNext,
                           Runnable onPrev, Runnable onNext) {
        net.minecraft.client.gui.components.Button prev =
                net.minecraft.client.gui.components.Button.builder(Component.literal("§l◀"), b -> onPrev.run())
                .bounds(left, y, 40, 20).build();
        prev.active = canPrev.getAsBoolean();
        addRenderableWidget(prev);

        net.minecraft.client.gui.components.Button label =
                net.minecraft.client.gui.components.Button.builder(
                Component.literal("§7" + (page + 1) + "/" + totalPages), b -> {})
                .bounds(left + 45, y, 44, 20).build();
        label.active = false;
        addRenderableWidget(label);

        net.minecraft.client.gui.components.Button next =
                net.minecraft.client.gui.components.Button.builder(Component.literal("§l▶"), b -> onNext.run())
                .bounds(left + 94, y, 40, 20).build();
        next.active = canNext.getAsBoolean();
