...
public final class ScreenQuad {
    ...
    // ───────────── 实体管线黑背板（光影下 debugQuads 不被绘制，改用实体纹理黑） ─────────────

    private static final Identifier BLACK_TEXTURE_ID =
            Identifier.fromNamespaceAndPath("cinemaforyou", "black_panel");
    private static VideoFrameTexture blackTexture;
    private static RenderType blackRenderType;
    private static boolean blackTried = false;

    /** 惰性创建 1×1 黑纹理 + 实体管线 RenderType（渲染线程）。 */
    private static void ensureBlackPanel() {
        if (blackTried) return;
        blackTried = true;
        try {
            RenderSystem.assertOnRenderThread();
            VideoFrameTexture t = new VideoFrameTexture("cinema_black");
            t.init(1, 1);
            long ptr = t.level(0).getPointer();
            if (ptr != 0L) {
                IntBuffer dest = MemoryUtil.memIntBuffer(ptr, 1);
                dest.put(0, 0xFF000000); // 不透明黑（alpha 高字节布局与视频一致）
            }
            t.uploadAll();
            Minecraft.getInstance().getTextureManager().register(BLACK_TEXTURE_ID, t);
            blackTexture = t;
            RenderSetup setup = RenderSetup.builder(RenderPipelines.ENTITY_SOLID)
                    .withTexture("Sampler0", BLACK_TEXTURE_ID,
                            () -> RenderSystem.getSamplerCache()
                    ...
                    .setOutline(RenderSetup.OutlineProperty.NONE)
                    .createRenderSetup();
            blackRenderType = RenderType.create("cinemaforyou_black", setup);
        } catch (Throwable e) {
            org.slf4j.LoggerFactory.getLogger("CinemaForYou/ScreenQuad")
                    .error("[CinemaForYou] 黑背板纹理创建失败: {}", e.toString());
        }
    }

    // ───────────── 半透明白纹理（光影下选择框/预览用：debugQuads 在光影下不绘制） ─────────────

    private static final Identifier WHITE_TEXTURE_ID =
            Identifier.fromNamespaceAndPath("cinemaforyou", "white_panel");
    private static VideoFrameTexture whiteTexture;
    private static RenderType whiteRenderType;
    private static boolean whiteTried = false;

    /** 惰性创建 1×1 白纹理 + 半透明实体管线 RenderType（渲染线程）。 */
    private static void ensureWhitePanel() {
        if (whiteTried) return;
        whiteTried = true;
        try {
            RenderSystem.assertOnRenderThread();
            VideoFrameTexture t = new VideoFrameTexture("cinema_white");
            t.init(1, 1);
            long ptr = t.level(0).getPointer();
            if (ptr != 0L) {
                IntBuffer dest = MemoryUtil.memIntBuffer(ptr, 1);
                dest.put(0, 0xFFFFFFFF); // 不透明白（顶点色负责着色与透明度）
            }
            t.uploadAll();
            Minecraft.getInstance().getTextureManager().register(WHITE_TEXTURE_ID, t);
            whiteTexture = t;
            RenderSetup setup = RenderSetup.builder(RenderPipelines.ENTITY_TRANSLUCENT)
                    ...
                    .setOutline(RenderSetup.OutlineProperty.NONE)
                    .createRenderSetup();
            whiteRenderType = RenderType.create("cinemaforyou_box", setup);
        } catch (Throwable e) {
            org.slf4j.LoggerFactory.getLogger("CinemaForYou/ScreenQuad")
                    .error("[CinemaForYou] 白纹理创建失败: {}", e.toString());
        }
    }
    ...
    public static void render(
        ...
        // 让"方块光"自发光通道生效（室内无阳光也不会全黑）。
        final int uv2x = 240;
        final int uv2y = 240;

        // 1. 黑色背板：无视频帧时也要显示（光影下 debugQuads 不绘制，
        //    改用实体管线黑纹理，平面与曲面同形）。
        if (textureId == null) {
            if (blackRenderType == null) {
                ensureBlackPanel();
            }
            if (curved) {
                if (blackRenderType != null) {
                    submitGridTextured(submitCollector, poseStack, blackRenderType,
                            new GridData(displayVerts, curvedGrid.cols, curvedGrid.rows,
                                    curvedGrid.uEdge, curvedGrid.vEdge), 255, uv2x, uv2y,
                            cameraPos);
                    return;
                }
            ...
            if (blackRenderType != null) {
                submitFlatEntity(submitCollector, poseStack, blackRenderType,
                        displayVerts, 255, uv2x, uv2y);
                return;
            }
    ...
    /** 平面四边形（双面）实体管线提交，供黑背板等复用。 */
    private static void submitFlatEntity(SubmitNodeCollector sc, PoseStack pose, RenderType type,
                                         float[] verts, int brightness, int uv2x, int uv2y) {
        final float[] f = verts;
        sc.order(0).submitCustomGeometry(pose, type, (ps, vc) -> {
            int[][] order = {{0, 3, 6, 9}, {3, 0, 9, 6}};
            float[][] uvs = {{0, 1, 1, 1, 1, 0, 0, 0}, {1, 1, 0, 1, 0, 0, 1, 0}};
            for (int face = 0; face < 2; face++) {
                for (int k = 0; k < 4; k++) {
                    int v = order[face][k];
                    vc.addVertex(ps, f[v], f[v + 1], f[v + 2])
    ...
    private static GridData computeCurvedGrid(CinemaScreen screen, float[] base) {
        ...
        // 网格整体加密一倍：逐格差异（光照/采样导数）随格子变小而不可见
        cols = Math.min(320, cols * 2);
        rows = Math.min(240, rows * 2);
        // 光影下再加密一倍：弧两端每格覆盖的弧角是中间的两倍（格子感最重处），
        // 加密后两端格角差减半，残余的逐格差异被压到不可见
        if (isShaderPackActive()) {
            cols = Math.min(640, cols * 2);
            rows = Math.min(480, rows * 2);
        }
    ...
    /** 曲面网格：视频纹理双面提交（26.2 ENTITY 全属性格式）。
     *  UV 与角点一一绑定：u/v 按弧角均匀分布。
     *  法线恒定朝上 (0,1,0)：与平面屏完全一致——光影包按法线算太阳/环境光照，
     *  常数法线使整面光照均匀（平面屏无格子感的直接原因）；逐格真实法线反而会
     *  让光影的阴影/漫反射项随格变化产生格子感。
     *  光影下单面提交（朝向相机的那张，避免同深度双面竞争）；
     *  未开光影维持双面（背面也能看到画面）。 */
    private static void submitGridTextured(SubmitNodeCollector sc, PoseStack pose, RenderType type,
                                           GridData grid, int brightness, int uv2x, int uv2y,
                                           double[] cameraPos) {
        final float[] gv = grid.verts;
        final boolean singleFace = isShaderPackActive()
                && cameraPos != null && cameraPos.length >= 3;
        final double pxc = (cameraPos != null && cameraPos.length >= 3) ? cameraPos[0] : 0.0;
        final double pyc = (cameraPos != null && cameraPos.length >= 3) ? cameraPos[1] : 0.0;
        final double pzc = (cameraPos != null && cameraPos.length >= 3) ? cameraPos[2] : 0.0;
        sc.order(0).submitCustomGeometry(pose, type, (ps, vc) -> {
            for (int r = 0; r < grid.rows; r++) {
                ...
                for (int c = 0; c < grid.cols; c++) {
                    ...
                    if (singleFace) {
                        // 光影下每格只画"朝向相机"的那一张面：
                        // 两张同深度面在侧视/掠射处会逐像素互相竞争，形成与网格粗细无关的
                        // 固定格纹；单面提交后该竞争消失。背侧观看时自动选背面绕向+镜像UV。
                        float e1x = gv[br] - gv[bl], e1y = gv[br + 1] - gv[bl + 1],
                                e1z = gv[br + 2] - gv[bl + 2];
                        float e2x = gv[tl] - gv[bl], e2y = gv[tl + 1] - gv[bl + 1],
                                e2z = gv[tl + 2] - gv[bl + 2];
                        float gxx = e1y * e2z - e1z * e2y;
                        float gyy = e1z * e2x - e1x * e2z;
                        float gzz = e1x * e2y - e1y * e2x;
                        float cxx = (gv[bl] + gv[br] + gv[tr] + gv[tl]) * 0.25f;
                        float cyy = (gv[bl + 1] + gv[br + 1] + gv[tr + 1] + gv[tl + 1]) * 0.25f;
                        float czz = (gv[bl + 2] + gv[br + 2] + gv[tr + 2] + gv[tl + 2]) * 0.25f;
                        boolean facesCam = gxx * (pxc - cxx) + gyy * (pyc - cyy)
                                + gzz * (pzc - czz) >= 0;
                        int[] order = facesCam ? orders[0] : orders[1];
                        float[] uvOfCorner = facesCam ? cornerUvFront : cornerUvBack;
                        for (int k = 0; k < 4; k++) {
                            int corner = order[k];
     ...
     * 不开光影走 debugQuads（纯顶点色）；开光影时 debugQuads 不被绘制，
     * 改走半透明实体管线 + 1×1 白纹理，用顶点色染色（与黑背板同理）。
     *
     * @param box       盒体（世界坐标）
     * @param r         红 0-255
     * @param g         绿 0-255
    ...
    public static void renderBox(
        ...
        if (isShaderPackActive()) {
            if (whiteRenderType == null) {
                ensureWhitePanel();
            }
            if (whiteRenderType == null) {
                return; // 创建失败时不画，避免崩溃
            }
            RenderType type = whiteRenderType;
            submitCollector.order(0).submitCustomGeometry(
                    poseStack, type,
                    (pose, vc) -> {
                        quadTex(vc, pose, minX, minY, maxZ, maxX, minY, maxZ, maxX, maxY, maxZ, minX, maxY, maxZ, r, g, b, alpha);
                        quadTex(vc, pose, maxX, minY, minZ, minX, minY, minZ, minX, maxY, minZ, maxX, maxY, minZ, r, g, b, alpha);
                        quadTex(vc, pose, maxX, minY, maxZ, maxX, minY, minZ, maxX, maxY, minZ, maxX, maxY, maxZ, r, g, b, alpha);
                        quadTex(vc, pose, minX, minY, minZ, minX, minY, maxZ, minX, maxY, maxZ, minX, maxY, minZ, r, g, b, alpha);
                        quadTex(vc, pose, minX, maxY, maxZ, maxX, maxY, maxZ, maxX, maxY, minZ, minX, maxY, minZ, r, g, b, alpha);
                        quadTex(vc, pose, minX, minY, minZ, maxX, minY, minZ, maxX, minY, maxZ, minX, minY, maxZ, r, g, b, alpha);
                    });
            return;
        }
    ...
    /** 提交单个四边形（ENTITY 全属性格式：白纹理染色 + 全亮光照 + 常数法线）。 */
    private static void quadTex(
            VertexConsumer vc, com.mojang.blaze3d.vertex.PoseStack.Pose pose,
            double x1, double y1, double z1,
            double x2, double y2, double z2,
            double x3, double y3, double z3,
        ...
        vc.addVertex(pose, (float) x1, (float) y1, (float) z1)
                .setColor(r, g, b, a).setUv(0.5f, 0.5f).setUv1(0, 10)
                .setUv2(240, 240).setNormal(0, 1, 0);
        ...
        vc.addVertex(pose, (float) x3, (float) y3, (float) z3)
                .setColor(r, g, b, a).setUv(0.5f, 0.5f).setUv1(0, 10)
                .setUv2(240, 240).setNormal(0, 1, 0);
        vc.addVertex(pose, (float) x4, (float) y4, (float) z4)
                .setColor(r, g, b, a).setUv(0.5f, 0.5f).setUv1(0, 10)
                .setUv2(240, 240).setNormal(0, 1, 0);
    }
    ...
    /** 所有顶点统一平移（相机外推）。 */
    /** 反射探测 Iris 光影是否启用（每 500ms 一次，无 Iris 返回 false）。 */
    private static Boolean shaderActiveCache = null;
    private static long shaderProbeAtMs = 0L;

    private static boolean isShaderPackActive() {
        long now = System.currentTimeMillis();
        if (shaderActiveCache != null && now - shaderProbeAtMs < 500L) {
            return shaderActiveCache;
        }
        shaderProbeAtMs = now;
        try {
            ...
            shaderActiveCache = Boolean.TRUE.equals(inUse);
        } catch (Throwable t) {
            shaderActiveCache = false;
        }
        return shaderActiveCache;
    }
