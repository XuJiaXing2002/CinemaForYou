...
public class ClientScreenManager {
    ...
    /**
     * 屏幕创建基线（首次见到该屏时的两角点）：移动/拉缩显示累计量以此为基点。
     * 键 = 屏幕 UUID；值 = [corner1, corner2]。
     */
    private final Map<UUID, BlockPos[]> screenBase = new HashMap<>();

    private void ensureBase(UUID id, CinemaScreen s) {
        if (s == null) return;
        if (!screenBase.containsKey(id)) {
            screenBase.put(id, new BlockPos[]{s.corner1().immutable(), s.corner2().immutable()});
        }
    }

    /** 该屏的创建基线两角点（从未记录过时返回 null）。 */
    public BlockPos[] baselineOf(UUID id) {
        return screenBase.get(id);
    }
    ...
    public void handleSync(ScreenSyncPayload payload) {
        ...
        for (CinemaScreen s : newMap.values()) {
            ensureBase(s.id(), s); // 服务器首次广播的角落即该屏创建基线
        }
